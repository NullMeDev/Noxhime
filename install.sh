...
# IMPORTANT: This script assumes the repository files are already present in the installation directory
...
install_prerequisites() {
  ...
  PACKAGES_TO_INSTALL=()
  REQUIRED_PACKAGES=("curl" "sqlite3" "build-essential" "libsqlite3-dev")
...
setup_repository() {
  ...
  if [ ! -f "package.json" ]; then
    error "The directory $INSTALL_DIR does not appear to contain valid Noxhime application files. Please ensure you have the correct files in this directory."
  fi
...
EOF
  ...
  echo
  log "NOTE: This script requires that you have already downloaded the application files to the installation directory."
  echo
