...
exports.getSecurityHardening = getSecurityHardening;
...
class SecurityHardening {
    constructor(client, notifyChannelId, logEvent, resultsDir) {
        ...
        this.resultsDir = resultsDir || path_1.default.join(process.cwd(), 'data', 'security-hardening');
     ...
     */
    async runSecurityCheck(profile = 'cis-level1') {
     ...
     */
    async runHardening(profile = 'cis-level1', applyFixes = true) {
        if (this.isHardening) {
        ...
        try {
            ...
            await this.logEvent('SECURITY_HARDENING', `Security hardening completed: ${result.summary.passed}/${result.summary.total_checks} checks passed, ` +
     ...
     */
    async reportHardeningResults(result, applyFixes) {
        try {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .addFields({ name: 'Profile', value: result.profile, inline: true }, { name: 'Duration', value: `${result.duration} seconds`, inline: true }, { name: 'Total Checks', value: result.summary.total_checks.toString(), inline: true }, { name: 'Passed', value: result.summary.passed.toString(), inline: true }, { name: 'Failed', value: result.summary.failed.toString(), inline: true }, { name: 'Warnings', value: result.summary.warnings.toString(), inline: true });
            if (applyFixes) {
                embed.addFields({ name: 'Fixes Applied', value: result.summary.fixes_applied.toString(), inline: true }, { name: 'Fixes Failed', value: result.summary.fixes_failed.toString(), inline: true });
            }
     ...
     */
    async checkCompliance(benchmark = 'cis-level1') {
        try {
            // Run a check without applying fixes
...
let securityHardeningInstance = null;
function getSecurityHardening(client, notifyChannelId, logEvent, resultsDir) {
    if (!securityHardeningInstance) {
        securityHardeningInstance = new SecurityHardening(client, notifyChannelId, logEvent, resultsDir);
    }
