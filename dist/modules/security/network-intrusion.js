...
exports.NetworkIntrusionDetection = void 0;
exports.getNetworkIntrusionDetection = getNetworkIntrusionDetection;
...
class NetworkIntrusionDetection {
    constructor(client, notifyChannelId, logEvent, alertLogPath) {
        this.checkInterval = null;
        this.lastReadPosition = 0;
        this.isInitialized = false;
        this.detectionEngine = 'none';
        this.client = client;
    ...
    async initialize() {
        try {
            ...
            if (this.detectionEngine !== 'none') {
                try {
                    await fs_1.default.promises.access(this.alertLogPath, fs_1.default.constants.R_OK);
                    // Get the current file size for tracking position
                    const stats = await fs_1.default.promises.stat(this.alertLogPath);
                    ...
                    const mockPath = path_1.default.join(process.cwd(), 'data', 'mock', `${this.detectionEngine}-alerts.log`);
                    const mockDir = path_1.default.dirname(mockPath);
                    if (!fs_1.default.existsSync(mockDir)) {
                        fs_1.default.mkdirSync(mockDir, { recursive: true });
                    }
                    if (!fs_1.default.existsSync(mockPath)) {
                        fs_1.default.writeFileSync(mockPath, ''); // Create empty file
                    }
                ...
                const mockPath = path_1.default.join(process.cwd(), 'data', 'mock', 'ids-alerts.log');
                const mockDir = path_1.default.dirname(mockPath);
                if (!fs_1.default.existsSync(mockDir)) {
                    fs_1.default.mkdirSync(mockDir, { recursive: true });
                }
    ...
    async checkAlerts() {
        try {
            ...
            const fileHandle = await fs_1.default.promises.open(this.alertLogPath, 'r');
        ...
        try {
            ...
            if (criticalAlerts.length > 0) {
                ...
                for (const alert of criticalAlerts) {
                    await this.logEvent('NETWORK_ALERT', `CRITICAL: ${alert.signature} from ${alert.source_ip} to ${alert.destination_ip}`);
                }
            }
            // Send high alerts
            if (highAlerts.length > 0) {
                ...
                await this.logEvent('NETWORK_ALERT', `HIGH: ${highAlerts.length} alerts including ${alertsToShow[0].signature}`);
            }
            ...
            if (otherAlerts.length > 0) {
                ...
                await channel.send({ embeds: [embed] });
                // Log summary
                await this.logEvent('NETWORK_ALERT', `SUMMARY: ${otherAlerts.length} medium/low alerts detected`);
            }
        }
        catch (error) {
...
exports.NetworkIntrusionDetection = NetworkIntrusionDetection;
// Export singleton instance
let networkIntrusionInstance = null;
function getNetworkIntrusionDetection(client, notifyChannelId, logEvent, alertLogPath) {
    if (!networkIntrusionInstance) {
        networkIntrusionInstance = new NetworkIntrusionDetection(client, notifyChannelId, logEvent, alertLogPath);
    }
